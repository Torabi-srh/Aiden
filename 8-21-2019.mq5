...
input bool     work_alt=false;                                                 // Work with a position in case of an opposite signal
...
input bool     BollingerBandsStrategy=false;                                   // Enable/disable Bollinger Bands Strategy
...
input int      RSIoverSold=25;
input int      RSImidPoint=50;
input int      RSIoverBought=75;
input int      BBlength=200;
input int      BBmult=2;

/*
//--- Short-Term Bollinger Reversion Strategy 
input string  ShortTermBollingerReversionStrategy_="Short-Term Bollinger Reversion Strategy";                      // Short-Term Bollinger Reversion Strategy
input bool ShortTermBollingerReversionStrategy=true;                                            // Enable/disable Short-Term Bollinger Reversion Strategy
*/
...
input int      BreathLevel_M5=2;                                               // 5 Minutes TimeFrame         
input int      BreathLevel_M10=5;                                              // 10 Minutes TimeFrame
input int      BreathLevel_M15=7;                                              // 15 Minutes TimeFrame
input int      BreathLevel_M30=15;                                             // 30 Minutes TimeFrame
...
//--- check the working time
   ...
   int _Buy=0,_Sell=0,_None=0,_R=0;
   _Buy+=(s1==buy && BollingerBandsStrategy?1:0);
   _None += (s1==none && BollingerBandsStrategy?1:0);
   _Sell += (s1==sell && BollingerBandsStrategy?1:0);
   _Buy+=(s2==buy && RSIBollingerBandsStrategy?1:0);
   _None += (s2==none && RSIBollingerBandsStrategy?1:0);
   _Sell += (s2==sell && RSIBollingerBandsStrategy?1:0);
   _Buy+=(s3==buy && MultiTimeFrameTrailingStop?1:0);
   _None += (s3==none && MultiTimeFrameTrailingStop?1:0);
   _Sell += (s3==sell && MultiTimeFrameTrailingStop?1:0);

   _R=MathMax(_Buy,MathMax(_None,_Sell));

   if(_R==_Buy) CurrentSignal = buy;
   if(_R==_Sell) CurrentSignal = sell;
   if(_R==_None) CurrentSignal = none;*/

   CurrentSignal=MultiTimeFrameCalculator(price_ask,price_bid);
      ...
      if(pos<1 && CuH>=breathLevel)
        {
         if(CurrentSignal==sell) // sell signal
           {
            StopLossByPivotPoint(price_ask,price_bid,PERIOD_M5,POSITION_TYPE_SELL);
         ...
         if(CurrentSignal==buy) // buy signal
           {
            StopLossByPivotPoint(price_ask,price_bid,PERIOD_M5,POSITION_TYPE_BUY);
...
signal SorSignal(double Ask,double Bid,ENUM_TIMEFRAMES period=PERIOD_M1)
  {
   ...
   if((vrsi>RSIoverSold && vrsi<RSImidPoint) && buyEntry)
     {
      return buy;
     }
   if((vrsi<RSIoverBought && vrsi>RSImidPoint) && sellEntry)
     {
      return sell;
     }
