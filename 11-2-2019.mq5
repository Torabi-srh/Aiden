...
double lowestlow = 0;
signal highlowstrategy() {
   MqlRates RateArray[];
   ArrayResize(RateArray, 1);
   if(!CopyRates(_symbol, _period, 0, 1, RateArray)) return none;
   if (highesthigh == 0) highesthigh = RateArray[0].high;
   if (lowestlow == 0)  lowestlow = RateArray[0].low;
   highesthigh = MathMax(RateArray[0].high, highesthigh);
   lowestlow = MathMin(RateArray[0].low, lowestlow);
