...
input ENUM_TIMEFRAMES      mainPeriod=PERIOD_M5;                               // Main Time Frame
...
signal GannStrategy(double Ask,double Bid,ENUM_TIMEFRAMES period=PERIOD_CURRENT)
  {
   ...
   int Gann=iCustom(_Symbol,period,"GannOP",period,50);
   int MACD=iMACD(_Symbol,period,23,50,17,PRICE_CLOSE);
      ...
      if(symbol==TradeSymbol && magic==magic_number)
        {
         ...
         double _MSL=((CurrentSL*MathSin(CurrentSL))+(MathPow(CurrentSL,2)))/(MathPow(CurrentSL,2)+CurrentSL*barNum+(CurrentSL*breathLevel/barNum));
         double _MPL=((CurrentTP*MathSin(CurrentTP))+(MathPow(CurrentTP,2)))/(MathPow(CurrentTP,2)+CurrentTP*barNum+(CurrentTP*breathLevel/barNum));
