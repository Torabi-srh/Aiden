...
double STMedianPrice[3];
void AddToSTMP(double v)
  {
   STMedianPrice[2]=STMedianPrice[1];
   STMedianPrice[1]=STMedianPrice[0];
   STMedianPrice[0]=v;
  }
void CalculateFib(double zero, double hundred, double &FibArray[])
  {
   double Range = MathAbs(zero-hundred);
   ArrayResize(FibArray,4);
   FibArray[0]=zero+(Range*0.236);
   FibArray[1]=zero+(Range*0.382);
   FibArray[2]=zero+(Range*0.5);
   FibArray[3]=zero+(Range*0.618);
  }
signal SwayTrade()
  {
   static int cnt=0;
   ...
   int MA=iMA(_Symbol,PERIOD_M30,3,0,MODE_LWMA,PRICE_MEDIAN);

   double MAv[];

   ArraySetAsSeries(MAv,true);
   CopyBuffer(MA,0,0,3,MAv);


   ArrayResize(RateArray,3);
   if(!CopyRates(_Symbol,PERIOD_M30,0,3,RateArray))
      return none;
   double MedianP = (RateArray[0].high * RateArray[0].low * RateArray[0].close * RateArray[0].close) / 4;
   AddToSTMP(MedianP);
   if(cnt<5)
     {
      cnt++;
   ...
   viv = MathMax(MedianP,viv);
   if(viv < STMedianPrice[0] && STMedianPrice[0] < STMedianPrice[1] && MAv[0] > MAv[1] && MAv[1] > MAv[2])
     {
...
signal MACDTrade()
  {
   int MACD=iMACD(_Symbol,PERIOD_M30,12,26,9,PRICE_CLOSE);
   ...
   ArraySetAsSeries(MACDi,true);
   CopyBuffer(MACD,1,0,3,MACDi);

   int trend=-1;
   trend = (MACDv[1]<MACDv[2]?1:-1);
   trend = (MACDv[1]>MACDv[2]?0:-1);
   if(MACDv[0] > MACDv[1] && trend==1)
     {
      return buy;
     }
   if(MACDv[0] < MACDv[1] && trend==0)
     {
      return sell;
     }
   return none;
...
void OnTick()
  {
   ...
   int period_secondsM30=PeriodSeconds(PERIOD_M30);
   ...
   datetime new_time3=TimeCurrent()/period_secondsM30*period_secondsM30;
   if(current_chart.isNewBar(new_time))
      NC=true;
   if(current_chart2.isNewBar(new_time2))
      NCH1=true;
   if(current_chart2.isNewBar(new_time3))
      NCM30=true;
   ...
   signal MidCurrentSignal=none;
   if(NCH1)
      BigCurrentSignal=DayTrade();
   if(NC)
      CurrentSignal=ManualDem(CIN);
   if(NCM30)
      MidCurrentSignal = MACDTrade();
//---
   ...
   if(work==true && work_day==true)
     {
     OpenPositions();
      if(MidCurrentSignal==buy)
        {
         if(oMidPos<MidPos)
           {
            if(trade.Buy(cvolumeB,_Symbol,price_ask,price_ask-SL*_Point,price_ask+TP*_Point,"MD"))
              {
               OpenSignal=buy;
               TradePerDay++; 
              }
         ...
         if(MidCurrentSignal==sell)
           {
            if(oMidPos<MidPos)
              {
               if(trade.Sell(cvolumeS,_Symbol,price_bid,price_bid+SL*_Point,price_bid-TP*_Point,"MD"))
                 {
                  OpenSignal=sell;
                  TradePerDay++; 
                 }
      ...
      if(CurrentSignal==buy)
        {
         if(oLowPos<LowPos)
           {
            if(trade.Buy(cvolumeB,_Symbol,price_ask,price_ask-SL*_Point,price_ask+TP*_Point,"LD"))
              {
               OpenSignal=buy;
               TradePerDay++; 
              }
         ...
         if(CurrentSignal==sell)
           {
            if(oLowPos<LowPos)
              {
               if(trade.Sell(cvolumeS,_Symbol,price_bid,price_bid+SL*_Point,price_bid-TP*_Point,"LD"))
                 {
                  OpenSignal=sell;
                  TradePerDay++; 
                 }
      ...
      if(BigCurrentSignal==buy)
        {
         if(oHighPos<HighPos)
           {
            if(trade2.Buy(cvolumeB,_Symbol,price_ask,price_ask-(SL*500)*_Point,price_ask+(TP*500)*_Point,"HD"))
              {
               OpenSignal=buy;
               TradePerDay++; 
              }
         ...
         if(BigCurrentSignal==sell)
           {
            if(oHighPos<HighPos)
              {
               if(trade2.Sell(cvolumeS,_Symbol,price_bid,price_bid+(SL*500)*_Point,price_bid-(TP*500)*_Point,"HD"))
                 {
                  OpenSignal=sell;
                  TradePerDay++; 
                 }
