   ...
   if(work==true && work_day==true)
     {
      if(CurrentSignal==buy && HA==buy)
        {
         //if(pos<POSITIONS) trade.Buy(order_volume,_Symbol,price_ask,price_ask-SL*_Point,price_ask+TP*_Point,NULL);
         OpenSignal=buy;
        }
      else if(CurrentSignal==sell && HA==sell)
        {
         if(pos<POSITIONS) trade.Sell(order_volume,_Symbol,price_bid,price_bid+SL*_Point,price_bid-TP*_Point,NULL);
         OpenSignal=sell;
        }
      if(ArcherCurrentSignal==buy)
        {
         //if(pos<POSITIONS+1) trade2.Buy(order_volume,_Symbol,price_ask,price_ask-(SL*500)*_Point,price_ask+(TP*500)*_Point,"AR");
         OpenSignal=buy;
        }
      else if(ArcherCurrentSignal==sell)
        {
         if(pos<POSITIONS+1) trade2.Sell(order_volume,_Symbol,price_bid,price_bid+(SL*500)*_Point,price_bid-(TP*500)*_Point,"AR");
         OpenSignal=sell;
        }
     }
         ...
         if(PositionGetInteger(POSITION_TYPE)==POSITION_TYPE_BUY)
           {
            ...
            if(NSL>SLC)
              ...
              {
               if(trade.PositionModify(PositionTicket,NSL,TPC-100*_Point))
                 {
                  Print("error");
                 }
              }
      ...
      if(symbol==_Symbol)
        {
         ...
         if(PositionGetInteger(POSITION_TYPE)==POSITION_TYPE_BUY)
           {
            double NSL=NormalizeDouble(price_ask-50*_Point,_Digits);
            double NSL2=NormalizeDouble(price_ask-5*_Point,_Digits);
            if(NSL>SLC)
              {
               if(trade.PositionModify(PositionTicket,NSL,TPC+10*_Point))
                 {
                  Print("error");
                 }
              }
              ...
              {
               if(trade.PositionModify(PositionTicket,NSL,TPC-5*_Point))
                 {
                  Print("error");
                 }
              }
