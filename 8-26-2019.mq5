...
input int      BreathLevel_M10=30;                                              // 10 Minutes TimeFrame
input int      BreathLevel_M15=45;                                              // 15 Minutes TimeFrame
...
signal FiftyTwoHundred(double Ask,double Bid,ENUM_TIMEFRAMES period=PERIOD_CURRENT)
  ...
  if (period==PERIOD_CURRENT)period=_Period;
   int ib=iBars(_Symbol,period);
   if(ib<201) return none;
   double FiftyArray[],TwoHundredArray[];

   int Fifty=iMA(_Symbol,period,50,0,MODE_EMA,PRICE_CLOSE);
   int TwoHundred=iMA(_Symbol,period,200,0,MODE_EMA,PRICE_CLOSE);
...
void MultiTimeFrameCalculator(double price_ask,double price_bid)
  {
   ...
   signal s_M1=ParabolicSAR(0.04,0.1,period_M1),
   s_M5=ParabolicSAR(0.04,0.1,period_M5),
   s_M10=ParabolicSAR(0.04,0.1,period_M10),
   s_M15=ParabolicSAR(0.04,0.1,period_M15),
   s_M30=ParabolicSAR(0.04,0.1,period_M30);
...
signal ParabolicSAR(double step,double maximum,ENUM_TIMEFRAMES period)
  {
    if (period==PERIOD_CURRENT)period=_Period;
   int ib=iBars(_Symbol,period);
   if(ib<step+1) return none;
   double ParabArray[];

   int Parab=iSAR(_Symbol,period,step,maximum);

   ArraySetAsSeries(ParabArray,true); 
   CopyBuffer(Parab,0,0,3,ParabArray);

   double ic = iClose(_Symbol,period,1);
   if(ParabArray[0]==0 && ParabArray[1]==0) return none;
   if(ic>ParabArray[0] && ic>ParabArray[1] && ic>ParabArray[2])
     {
      return buy;
     }
   else if(ic<ParabArray[0] && ic<ParabArray[1] && ic<ParabArray[2])
     {
      return sell;
     } 
   return none;
      ...
      if(symbol==TradeSymbol && magic==magic_number)
        {
         ...
         double _MSL=CurrentSL+breathLevel/barNum;
         double _MPL=CurrentTP+(breathLevel/(2*barNum));
