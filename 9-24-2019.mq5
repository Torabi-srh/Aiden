...
//bool NC2=IsNewCandle(TradeSymbol);
   ...
   if(NC) LCurrentSignal=Archer(LS,PERIOD_H1);
   if(NC) HCurrentSignal=Archer(HS,PERIOD_H1);
   if(NC) MCurrentSignal=Archer(MS,PERIOD_H1);

   if(LCurrentSignal==buy || HCurrentSignal==buy || MCurrentSignal==buy)
     {
      CurrentSignal=buy;
     }
   if(LCurrentSignal==sell || HCurrentSignal==sell || MCurrentSignal==sell)
     {
      CurrentSignal=sell;
     }
//---
      ...
      if(pos<1)
        {
         ...
         if(CurrentSignal==buy)
         ...
         else if(CurrentSignal==sell)
           {
            ...
            trade.Sell(MMV, MS, Mprice_ask, MTP, MSL,"mid");
            trade.Sell(LLV, LS, Lprice_ask, LTP, LSL,"low");
            trade.Sell(HHV, HS, Hprice_ask, HTP, HSL,"high");
            OpenSignal=sell;
           }
        }
     }
      ...
      if(magic==magic_number)//symbol==_Symbol &&
        {
         ...
         if(tp==POSITION_TYPE_BUY)
           ...
           {
            if(AchillesKnee==1) AchillesKnee=HeikenAshiValue[0];
            double NSL=NormalizeDouble(CSL+(HeikenAshiValue[0]-AchillesKnee),_digits);
